      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm ring-1 ring-gray-200">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">
            Live Bins by Location (Bhubaneswar)
          </h3>
          <Pill label={`${bins.length} devices`} tone="info" />
        </div>
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="text-left text-gray-500 border-b">
                <th className="py-2 pr-4">Bin ID</th>
            ...
            <tbody>
              {bins.map((b) => {
                const tone =
                  b.fill >= 90 ? "danger" : b.fill >= 80 ? "warn" : "success";
                const autoEnabled =
                  (b.type === "Organic" || b.type === "Hazardous") &&
                  b.fill >= 80;
                const enabled =
                  autoEnabled ||
                  b.fill >= 90 ||
                  (b.fill >= 80 && b.type === "Recyclable");
                return (
                  <tr key={b.id} className="border-b last:border-0">
                    ...
                    <td className="py-3 pr-4 w-64">
                      <div className="flex items-center gap-3">
                        <Progress value={b.fill} />
                        <span className="w-12 text-right font-medium">
                          {percentToLabel(b.fill)}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="py-3 pr-4">
                      <button
                        ...
                        disabled={!enabled}
                        onClick={() => enabled && onClickDispatch(b)}
                      >
                        Dispatch Truck
                      </button>
                    </td>
