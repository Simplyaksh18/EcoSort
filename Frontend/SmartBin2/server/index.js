...
app.post('/api/dispatch', (req,res)=>{
  ...
  const station = Object.values(stations).flat().find(s=>s.id===stationId)
  if(!bin || !driver || !station) return res.status(400).json({ error:'Invalid bin/driver/station' })
  if(driver.status!=='Available') return res.status(409).json({ error:'Driver not available' })
...
app.listen(PORT, ()=>console.log(`Bhubaneswar server on http://localhost:${PORT}`))
